...
from pykrx.website.krx.future.core import (
    AllProductPrice,
    DerivativeProductSearch,
)


def get_future_ticker_and_name() -> DataFrame:
    return DerivativeProductSearch().fetch()


def get_future_ticker_list() -> DataFrame:
    return DerivativeProductSearch().fetch().index.to_list()  # type: ignore[return-value]
...
def get_future_ohlcv_by_ticker(date: str, prod: str) -> DataFrame:
    ...
    df = AllProductPrice().fetch(date, prod)
    df = df[
        [
            "ISU_SRT_CD",
            "ISU_NM",
