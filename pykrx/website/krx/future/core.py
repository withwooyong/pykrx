...
class DerivativeProductSearch(KrxWebIo):
    """파생상품검색 - 파생상품 종목을 검색하는 클래스"""
    ...
    def fetch(self) -> DataFrame:
        ...
        df = DataFrame(data["output"])  # type: ignore[index]
        return df.set_index("value")


class AllProductBasicInfo(KrxWebIo):
    """전종목기본정보 - 전체 파생상품 종목의 기본 정보를 조회하는 클래스"""

    @property
    def bld(self):
        return "dbms/MDC/STAT/standard/MDCSTAT12801"

    def fetch(self, product_id: str) -> DataFrame:
        """[15004] 전종목 기본정보

        Args:
            product_id   (str): 선물 티커
        ...
        if (
            (product_id == "KRDRVFUEQU")
            or (product_id == "KRDRVOPEQU")
            or (product_id == "KRDRVFUXAT")
        ):
            # Single Stock Futures
            # Single Stock Options
            # Secotr Index Futures
            sub_prod_id = str(product_id)

            result = self.read(prodId=product_id, subProdId=sub_prod_id, csvslx_isNo=False)
        else:
            result = self.read(prodId=product_id, csvslx_isNo=False)
        return DataFrame(result["output"])  # type: ignore[index]


class AllProductPrice(KrxWebIo):
    """전종목시세 - 전체 파생상품 종목의 시세를 조회하는 클래스"""

    @property
    def bld(self):
        return "dbms/MDC/STAT/standard/MDCSTAT12501"

    def fetch(self, trade_date: str, product_id: str) -> DataFrame:
        """[15001] 전종목 시세
        ...
        """  # pylint: disable=line-too-long # noqa: E501
        result = self.read(trdDd=trade_date, prodId=product_id, mktTpCd="T", rghtTpCd="T")
        return DataFrame(result["output"])  # type: ignore[index]


if __name__ == "__main__":
    pd.set_option("display.width", None)
    print(AllProductBasicInfo().fetch(product_id="KRDRVFUEQU"))
