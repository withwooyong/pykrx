    ...
    def _market_valid_check(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if "market" in sig.bind_partial(*args, **kwargs).arguments:
                valid_market_list = ["ALL", "KOSPI", "KOSDAQ", "KONEX"] if param is None else param
...
def resample_ohlcv(df, freq, how):
    ...
    if freq != "d" and len(df) > 0:
        if freq == "m":
            df = df.resample("ME").apply(how)  # 'M' is deprecated, use 'ME' instead
        elif freq == "y":
            df = df.resample("YE").apply(how)  # 'Y' is deprecated, use 'YE' instead
        else:
