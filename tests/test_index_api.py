...
class TestIndexTickerList:
    ...
    get_index_ticker_list() 함수를 테스트합니다.
    - 특정 일자의 지수 티커 리스트 조회
    - 휴일의 지수 티커 리스트 조회
    - KOSDAQ, 테마 지수 리스트 조회
    - 지수 티커 이름 조회
...
class TestIndexPortfolioDepositFile:
    """지수 포트폴리오 구성 종목 조회 관련 테스트

    get_index_portfolio_deposit_file() 함수를 테스트합니다.
    - 기본 파라미터로 지수 구성 종목 조회
    - 특정 영업일의 지수 구성 종목 조회
    - 2014년 5월 1일 이전 데이터 조회 (데이터 없음)
    """
...
class TestIndexOhlcvByDate:
    """지수 OHLCV(시가, 고가, 저가, 종가, 거래량) 조회 관련 테스트

    get_index_ohlcv_by_date() 함수를 테스트합니다.
    - 특정 기간의 지수 OHLCV 데이터 조회
    - 휴일 데이터 조회 (빈 DataFrame 반환)
    - 월별 데이터 리샘플링 조회
    - 과거 데이터 조회 (NaN 값 처리)
    """
...
class TestIndexListingDate:
    ...
    get_index_listing_date() 함수를 테스트합니다.
    - 지수별 기준시점, 발표시점, 기준지수, 종목수 정보 조회
    """
    ...
    def test_listing_info(self):
        ...
        assert df.loc["코스피", "발표시점"] == "1983.01.04"
        assert df.loc["코스피", "기준지수"] == 100.0


class TestIndexPriceChangeByTicker:
    """지수별 가격 변동 조회 관련 테스트

    get_index_price_change_by_ticker() 함수를 테스트합니다.
    - 유효한 영업일 기준 지수 가격 변동 조회
    - 시작일/종료일이 휴일인 경우 조회 (자동으로 영업일로 변환)
    """
